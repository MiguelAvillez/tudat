...
public:
    ...
    virtual StateType performIntegrationStep( const TimeStepType stepSize )
    {
        ...
        for ( int stage = 0; stage < this->butcherTableau_.cCoefficients.rows( ); stage++ )
        {
            ...
            currentScaledStateDerivatives_[ stage ] = stepSize * this->stateDerivativeFunction_(independentVariable, intermediateState );
            ...
            const IndependentVariableType time = this->timeFunction_( independentVariable, intermediateState );
